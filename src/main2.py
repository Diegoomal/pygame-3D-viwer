...
class Mesh:
    ...
    def __init__(self, faces:list[list[int]], vertices:list[list[float]], position=[0,0,0,1]):
        self.faces = np.array(faces, dtype=object)                              # int para object
...
class Renderer:
    ...
    def __init__(self, screen, width, height, texture=None):
        self.screen = screen
        self.width = width
        self.height = height
    ...
    def render(self, mesh, camera):
        ...
        verts = mesh.get_transformed() @ camera.get_view_matrix() @ camera.get_projection_matrix()
    ...
    def run(self):
        ...
        while True:
            ...
            for mesh in self.scene:
                ...
                self.renderer.render(mesh, self.camera)
...
if __name__=='__main__':
    ...
    faces, verts = FileManager('./assets/models/suzanne/model.obj').load()
